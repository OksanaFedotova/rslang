...
const Auchform: React.FunctionComponent<IAuchForm> = ({
  wordsCard,
  KeyCode
}) => {
  ...
  function CheckWord(propWord: string, propaudioWord: string, num: number) {
    if (propWord === propaudioWord) {
      document
        .querySelector(
          "#root > div > div.audiocard > div > div > div:nth-child(" +
            num +
            ") > b"
        )
    ...
    document
      .querySelector("#root > div > button.header-button.next.noclick")
    ...
    <div className="audiocard">
      <div className="wrapper-word">
        ...
        <div className="stat">
          <div>
            <i>
              Правильно: {countWin} | Не правильно: {countfail}
            </i>
          </div>
          <div>
            <i>Осталось слов: {toEnd}</i>
          </div>
        </div>
      </div>
        ...
        <div className="stat-wrapper">
          <div className="know-words">
            ...
            {knowWords.map((item, index) => {
              return index % 2 ? null : (
                <div className="small-word" key={`small${index}`}>
                  {" "}
                  {item}
                  <img
                    src={audioButton}
          ...
          <div className="know-words">
            ...
            {dontknowWords.map((item, index) => {
              return index % 2 ? null : (
                <div className="small-word" key={`smallword${index}`}>
                  {" "}
                  {item}
                  <img
                    src={audioButton}

...
const getVariants = (words: IWord[], callback: React.Dispatch<string[]>) => {
  const randomIndex = Math.floor(Math.random() * words.length);
  const word = words[randomIndex];
  const englishWord = word.word;
  const isItCorrectCombination = Math.floor(Math.random() * 2);
  const russianWord = isItCorrectCombination
    ? word.wordTranslate
...
const Sprint = () => {
  const groupPage = useSelector((state: RootState) => state.page);
  const groupCurrent = groupPage.currentGroup;
  const pageCurrent = groupPage.currentPage;
  ...
  useEffect(() => {
    if (groupCurrent !== null && pageCurrent !== null) {
      document
    ...
    <>
      ...
      <Layout title='Игра "Cпринт"'>
        <div className="game-description">
          ...
          <p>
            <b>Цель:</b> дать максимальное количество правильных ответов за 30
            секунд.
          </p>
        </div>
          ...
          <div className="level-block">
            <div
              ...
              onClick={() => {
                ...
                document
                  .querySelector(
                    "#root > div > div > div.level-block > div:nth-child(2)"
                  )
                  ?.classList.remove("active");
                document
                  .querySelector(
                    "#root > div > div > div.level-block > div:nth-child(3)"
                  )
                  ?.classList.remove("active");
                document
                  .querySelector(
                    "#root > div > div > div.level-block > div:nth-child(4)"
                  )
                  ?.classList.remove("active");
                document
                  .querySelector(
                    "#root > div > div > div.level-block > div:nth-child(5)"
                  )
                  ?.classList.remove("active");
                document
                  .querySelector(
                    "#root > div > div > div.level-block > div:nth-child(6)"
                  )
                  ?.classList.remove("active");
                document
                  .querySelector(
                    "#root > div > div > div.level-block > div:nth-child(1)"
                  )
                  ?.classList.add("active");
              }}
            >
              ...
              onClick={() => {
                ...
                }
                {
                  words.forEach(word => {
                    if (word.id == wordId) {
                      if (
                        word.word == variants[0] &&
                        word.wordTranslate == variants[1]
                      ) {
        ...
        <div className="result-wrapper">
            ...
            onClick={() => {
              if (user) {
                setStatistic(
                  user,
                  "Sprint",
                  pushStat(correctAnswers),
                  pushStat(wrongAnswers),
                  maxSerie.current
                );
              }
              document
          ...
          >
            ...
            <button className="header-button" onClick={() => navToPage("..")}>
              На главную
            </button>
            <button
              className="header-button"
              onClick={() => navToPage("../textbook")}
            >
              В учебник
            </button>
          </div>
